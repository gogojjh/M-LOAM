...
void process()
{
    while (1)
    {
               ...
               !full_res_buf.empty() && !ext_buf.empty() && !odometry_buf.empty())
        {
            ...
            if (pose_wmap_curr.t_.x() + CUBE_HALF < 0)
                center_cub_i--;
            if (pose_wmap_curr.t_.y() + CUBE_HALF < 0)
                center_cub_j--;
            if (pose_wmap_curr.t_.z() + CUBE_HALF < 0)
                center_cub_k--;
            ...
            for (auto &point : *laser_cloud_full_res)
                pointAssociateToMap(point, point, pose_wmap_curr);
